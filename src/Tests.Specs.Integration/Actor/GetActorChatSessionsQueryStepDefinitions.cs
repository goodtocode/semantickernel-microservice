...
public class GetActorChatSessionsQueryStepDefinitions : TestBase
{
    ...
    public async Task WhenIGetTheChatSessions()
    {
        if (_exists)
        {
            ...
            var chatSession = ChatSessionEntity.Create(_id, actor.Id, "Test Session", ChatMessageRole.assistant, "First Message", "First Response");
        ...
        if (validationResponse.IsValid)
            try
            {
                var handler = new GetAuthorChatSessionsByOwnerIdQueryHandler(context);
